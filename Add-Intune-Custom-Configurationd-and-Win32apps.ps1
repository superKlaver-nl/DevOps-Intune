...
foreach ($appItem in $apps){
    ...
    #    $FileRule = New-DetectionRule -File -Path ($($appData.DetectionRules.path)) -FileOrFolderName ($($appData.DetectionRules.fileOrFolderName)) -FileDetectionType ($($appData.DetectionRules.detectionType)) -check32BitOn64System ($($appData.DetectionRules.check32BitOn64System))
         $FileRule = New-DetectionRule -File -Path ($($appData.DetectionRules.path)) -FileOrFolderName ($($appData.DetectionRules.fileOrFolderName)) -FileDetectionType ($($appData.DetectionRules.detectionType)) -FileDetectionValue ($($appData.DetectionRules.detectionValue)) -operator ($($appData.DetectionRules.operator)) -check32BitOn64System ($($appData.DetectionRules.check32BitOn64System))
    }
